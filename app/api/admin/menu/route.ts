...
const sizeSchema = z.object({ id: z.string().uuid().optional(), label: z.string().trim().min(1).max(40), pricePaise: z.number().int().min(0).max(10_000_000), extraCheesePricePaise: z.number().int().min(0).max(100_000), isAvailable: z.boolean() })
const itemSchema = z.object({
  id: z.string().uuid().optional(), name: z.string().trim().min(2).max(100), description: z.string().trim().max(500),
...
export async function PATCH(request: Request) {
  try {
    await requireAdmin(); const input = itemSchema.extend({id:z.string().uuid()}).parse(await request.json())
    if (input.defaultSizeIndex >= input.sizes.length) throw new ApiError(400, 'Choose a valid default size.', 'INVALID_DEFAULT_SIZE')
...
export async function DELETE(request:Request){try{await requireAdmin();const {id}=z.object({id:z.string().uuid()}).parse(await request.json());const {error}=await createAdminClient().from('menu_items').update({is_available:false,archived_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',id);if(error)throw error;return NextResponse.json({success:true})}catch(error){return apiErrorResponse(error)}}
